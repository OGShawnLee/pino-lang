  ...
  static std::string transpile_statement(Statement *statement, size_t indent = 0) {
    ...
    switch (statement->kind) {
      ...
      case StatementType::LOOP_STATEMENT: {
        ...
        if (loop->limit->type == ExpressionType::IDENTIFIER) {
          Identifier *identifier = static_cast<Identifier *>(loop->limit.get());
          limit_var_name = identifier->name;
        } else {
          Value *value = static_cast<Value *>(loop->limit.get());
          limit_var_name = value->value;
        }

        if (loop->loop_type == LoopType::TIMES_LOOP) {
          output += 
            indentation + "for (let " + index_var_name + " = 0; " + index_var_name + " < " + limit_var_name + "; " + index_var_name + "++) {\n";
        } else {
