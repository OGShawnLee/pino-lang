...
class Parser {
  ...
  forStatement() {
    ...
    // Otherwise, it could be for time in 5 { ... } or for item in vector { ... } or for key, val in dict
    ...
    if (this.check(TokenType.DELIMITER, ',')) {
      ...
      keyVar = varName;
      const valToken = this.consume(TokenType.IDENTIFIER, "Expect value variable name after ','");
      varName = valToken.value;
    }

    this.consume(TokenType.KEYWORD, "Expect 'in' after iterator variable(s)", 'in');
    ...
    const body = this.block();
    this.popScope();
    return new ForStmt(varName, iterableExpr, body, false, keyVar);
  }
...
export { Lexer, Parser, Environment, PinoModule, Interpreter, TypeChecker, runPinoCode };
